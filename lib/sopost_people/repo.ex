defmodule SopostPeople.Repo do
  ...
  def get(module, id) do
    Enum.find all(module), fn p -> p.id == id end
  end

  def get_by(module, params) do
    Enum.find all(module), fn p ->
      Enum.all?(params, fn {key, val} -> Map.get(p, key) == val end)
    end
  end

  def get_by_location(module, location) do
    Enum.filter all(module), fn p ->
      Enum.any?(p.locations, fn l -> l == location end)
    end
  end
end
